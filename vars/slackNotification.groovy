...
def call(String buildResult, String channel) {
  ...
  else if( buildResult == "COVERAGE" ) { 
    slackSend color: '#909090',channel: channel, message: "*Job:* `${env.JOB_NAME}` with buildnumber *${env.BUILD_NUMBER}* coverage reports. \nAccess <http://54.208.64.4:8080/job/${env.JOB_NAME}/${env.BUILD_NUMBER}/jacoco/|here>."
  } 
  else {
